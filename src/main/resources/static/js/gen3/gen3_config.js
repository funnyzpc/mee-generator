...
var module={
    ...
    api:{
        /* 列表分页查询：GET请求 */
        page: ctxPath+"/code_gen3_config/list",
        /* 添加：POST请求 */
        add: ctxPath+"/code_gen3_config/add",
        /* 删除: DELETE请求 */
        del: ctxPath+"/code_gen3_config/delete",
        /* 修改: PUT请求 */
        mod: ctxPath+"/code_gen3_config/update",
...
function doDelete(data,idx, elem){
    ...
    FetchUtils.fetchDelete(ctxPath+"/code_gen3_config/delete?id="+data.id,"{}",function(res){
       if(!res || res.status!==1){
...
function doPreview(item,idx,elem){
    ...
    window.open(ctxPath+'/code_gen3_config/gen3_preview.html?table_id='+item.table_id,'_blank','channelmode=yes,fullscreen=yes')
};

// 修改添加生成配置
function doNewConfig(){
    let win = window.open(ctxPath+'/code_gen3_config/gen3_config_form.html?type=1','_blank','channelmode=yes,width=800px,left=320px,top=80px');
    // 关闭窗口后事件回调
...
function doEditConfig(data,idx,elem){
    let win = window.open(ctxPath+'/code_gen3_config/gen3_config_form.html?type=2&id='+data.id,'_blank','channelmode=yes,width=800px,left=320px,top=80px');
    // 关闭窗口后事件回调
    win.onbeforeunload = function(){
        // 回调后执行一次查询
        Common.doQuery();
...
function doEnable(data,idx,elem){
    let table_id = data.table_id;
    FetchUtils.fetchPost(ctxPath+"/code_gen3_config/enable",data,function(res){
        if(!res || res.status!==1){
            alert(!res?"操作失败":res.msg);
        }
         // 回调后执行一次查询
