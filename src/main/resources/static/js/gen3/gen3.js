...
var module={
    ...
    api:{
        /* 列表分页查询：GET请求 */
        "page": ctxPath+"/code_gen3/list",
        /* 添加：POST请求 */
        add: ctxPath+"/code_gen3/add",
        /* 删除: DELETE请求 */
        del: ctxPath+"/code_gen3/delete",
        /* 修改: PUT请求 */
        mod: ctxPath+"/code_gen3/update",
...
function doDelete(data,idx, elem){
    FetchUtils.fetchDelete(ctxPath+"/code_gen3/"+data.id,"",function(res){
        //alert(res);
         // 回调后执行一次查询
         Common.doQuery();
    });
...
function doPreview(item,idx,elem){
     ...
     debugger
    window.open(ctxPath+'/code_gen3/gen3_preview.html?table_id='+item.id,'_blank','channelmode=yes,fullscreen=yes')
};

// 导入表
function doImportTable(){
    let win = window.open(ctxPath+'/code_gen3/gen3_import.html','_blank','channelmode=yes,width=800px,left=320px,top=80px');
    // 关闭窗口后事件回调
...
function edit(table_id){
    window.open(ctxPath+'/code_gen3/gen3_edit.html?table_id='+item.table_id,'_blank','channelmode=yes,fullscreen=yes')
};
