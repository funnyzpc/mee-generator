...
var module={
    ...
    default_query:true,
    /* 分页表单数据(可选) */
    // page_form: {"page_no":0,"page_size":4},
    ...
    api:{
        /* 列表分页查询：GET请求 */
        "page": ctxPath+"/code_gen1/list",
        /* 添加：POST请求 */
        "add": ctxPath+"/code_gen1/add",
        /* 删除: DELETE请求 */
        "delete": ctxPath+"/code_gen1/delete",
        /* 修改: PUT请求 */
        "mod": ctxPath+"/code_gen1/update",
...
function doDelete(data,idx, elem){
    ...
    FetchUtils.fetchDelete(ctxPath+"/code_gen1/"+table_id,"",function(res){
        //alert(res);
         // 回调后执行一次查询
         Common.doQuery();
    });
...
function doPreview(item,idx,elem){
    ...
    window.open(ctxPath+'/code_gen1/gen1_preview.html?table_id='+item.table_id,'_blank','channelmode=yes,fullscreen=yes')
};

// 导入表
function doImportTable(){
    let win = window.open(ctxPath+'/code_gen1/gen1_import.html','_blank','channelmode=yes,width=800px,left=320px,top=80px');
    // 关闭窗口后事件回调
...
function edit(table_id){
    window.open(ctxPath+'/code_gen1/gen1_edit.html?table_id='+item.table_id,'_blank','channelmode=yes,fullscreen=yes')
};
